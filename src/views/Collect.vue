  ...
  <div class="article_contanier">
    <div v-if="ready">
      ...
      <div class="container_placeholder" v-else>
        您还没有收藏过文章，快去<router-link to="/">看看</router-link>吧
      </div>
...
export default {
  ...
  created() {
    document.title = 'Article-我的收藏';
    this.init();
  },
  methods: {
    init() {
      if(this.$store.state.userAddress) {
        this.queryCollect(this.currentPage);
      } else {
    ...
    refresh() {
      ...
      // this.queryCollect(1, false);
    },
    queryCollect(currentPage, needConcat = true) {
      this.loading = true;
      var value = "0";
      var nonce = "0";
