  ...
  <div class="article_contanier">
    <div v-if="ready">
      ...
      <div class="container_placeholder" v-else>
        您还没有发表过文章，快去<router-link to="/publish">发表</router-link>一篇吧！
      </div>
...
export default {
  ...
  created() {
    document.title = 'Article-我的文章';
    this.init();
  },
  methods: {
    init() {
      if(this.$store.state.userAddress) {
        this.queryMyArticle(this.currentPage);
      } else {
    ...
    refresh() {
      ...
      // this.queryMyArticle(1, false);
    },
    queryMyArticle(currentPage, needConcat = true) {
      this.loading = true;
      var value = "0";
      var nonce = "0";
