  ...
  <div class="article_contanier">
    <div v-if="ready">
      ...
      <div class="container_placeholder" v-else>
        您还没有购买过文章，快去<router-link to="/">看看</router-link>吧！
      </div>
...
export default {
  ...
  created() {
    document.title = 'Article-我的订单';
    this.init();
  },
  methods: {
    init() {
      if(this.$store.state.userAddress) {
        this.queryBuy(this.currentPage);
      } else {
    ...
    refresh() {
      ...
      // this.queryBuy(1, false);
    },
    queryBuy(currentPage, needConcat = true) {
      this.loading = true;
      var value = "0";
      var nonce = "0";
